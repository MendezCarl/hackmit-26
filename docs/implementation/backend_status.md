...
The host baseline is `88d7bbb`, including mocked recovery, sessions, signals,
transcripts, professor summaries, and WebSockets.
...
[ADR 0002](../decisions/adr_0002_learning_plan_contracts.md) for how the new
plans were reconciled with earlier shared contracts.

The complete synthetic flow runs on the recovery-flow feature branch. Isolated
feature branches export independently tested services and routers; host
composition owns their integration. No live provider call, webcam capture,
model download or frontend modification was performed.

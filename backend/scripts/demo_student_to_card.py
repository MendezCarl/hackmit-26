...
TRANSCRIPT = [
    ...
    (120_000, 180_000, "Recursion uses the call stack to remember where each call returns."),
]
...
def run_demo(signals: list[StudentSignal]) -> dict[str, object]:
    ...
    token = issue_access_token(settings, AuthenticatedActor(user_id="student-1", role="student"))
    headers = {"Authorization": f"Bearer {token}"}
    session_id = client.post(
        "/api/v1/sessions",
        json={"lecture_id": LECTURE_ID, "course_id": "course-1", "title": "Synthetic stacks lecture", "mode": "in_person"},
        headers=headers,
    ).json()["session_id"]
    client.post(
        ...
        json={
            ...
            "chunks": [
                {"chunk_id": f"chunk-{index}", "session_id": session_id, "start_ms": start, "end_ms": end,
                 "text": text, "source": "local_transcription", "is_final": True, "revision": 1}
                for index, (start, end, text) in enumerate(TRANSCRIPT, start=1)
            ],
        },
        headers=headers,
    ).raise_for_status()
    receipt = post_signals(client, session_id, LECTURE_ID, signals, token, LECTURE_OFFSET_MS)
    first = signals[0]
    job = client.post(
    ...
    card = client.get(
        f"/api/v1/sessions/{session_id}/recovery/cards/{job.json()['card_id']}", headers=headers
    )
...
def main() -> None:
    ...
    source.add_argument("--signals-file", type=Path, help="JSON Lines from the student worker.")
    ...
    if arguments.clip:
        ...
        analyzer = OnnxStudentAnalyzer.from_files(
            Path("models/person_detector.onnx"), Path("models/face_detection_yunet_2023mar.onnx")
        )
        policy = StudentSignalPolicy()
        summary = analyze_clip(StudentSignalWorker(analyzer, policy), arguments.clip, policy.sampling_ms)
    ...
    for signal in signals:
        print(f"signal: {signal.event_type:18} {signal.start_ms / 1000:.1f}-{signal.end_ms / 1000:.1f}s confidence {signal.confidence:.2f}")
    result = run_demo(signals)
    receipt, card = result["receipt"], result["card"]
    print(f"backend accepted {len(receipt['accepted_event_ids'])} signal(s); "  # type: ignore[index]
          f"advisory eligible list: {len(receipt['recovery_eligible_event_ids'])}")  # type: ignore[index]
    print("student asks: 'I missed that'  ->  recovery job", result["job"]["status"])  # type: ignore[index]
    print(json.dumps({k: card[k] for k in ("topic", "what_you_missed", "key_facts", "source_timestamps") if k in card}, indent=2))  # type: ignore[index,union-attr]
    print("(card from the deterministic mock generator, grounded in the synthetic transcript; not OpenAI)")

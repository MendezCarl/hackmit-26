# Feature 4: professor-aggregation
...
Branch: `feature/backend-professor-aggregation`. Base: `backend`.

Professor-only post-lecture summaries with fixed buckets, unique consenting participants, suppression and compassionate recap suggestions. Upstream events are mocked in isolation tests.
...
```sh
backend/venv/bin/python -m pytest backend/tests backend/feature_tests/professor_aggregation
...
Contracts: `shared/contracts/professor_aggregation.schema.json` and `docs/api/professor_aggregation.openapi.json`.
